#    This file is a part of the ForceSub distribution (https://github.com/ShikariBaaZ/ForceSub_Bot).
#    Copyright (c) 2021 Shikari
#    
...
# 
#    License can be found in < https://github.com/ShikariBaaZ/ForceSub_Bot> .
...
if (apiid != None and apihash!= None and bottoken != None):
    try:
        Shikari = TelegramClient('Shikari', apiid, apihash).start(bot_token=bottoken)
...
async def get_user_join(id):
    ok = True
    try:
        await Shikari(GetParticipantRequest(channel=channel, participant=id))
        ok = True
...
async def _(event):
    ...
    if event.user_joined or event.user_added:
        ...
        pp = await Shikari.get_participants(chat)
        count = len(pp)
        ...
        if x is True:
            ...
            await Shikari.edit_permissions(event.chat.id, user.id, until_date=None, send_messages=False)
        
        await event.reply(msg, buttons=butt)


@Shikari.on(events.NewMessage(incoming=True))
async def mute_on_msg(event):
    ...
    temp = await Shikari(GetFullUserRequest(event.sender_id))
    if x is False:
        if temp.user.bot:
            return
        nm = temp.user.first_name
        try:
            await Shikari.edit_permissions(event.chat.id, event.sender_id, until_date=None, send_messages=False)
        except Exception as e:
            print(str(e))
            return
...
@Shikari.on(events.callbackquery.CallbackQuery(data=re.compile(b"unmute_(.*)")))
async def _(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    if uid == event.sender_id:
        x = await get_user_join(uid)
        nm = (await Shikari(GetFullUserRequest(uid))).user.first_name
        if x is False:
            await event.answer(f"You haven't joined @{channel} yet!", cache_time=0, alert=True)
        elif x is True:
            try:
                await Shikari.edit_permissions(event.chat.id, uid, until_date=None, send_messages=True)
            except Exception as e:
                print(str(e))
                return
...
@Shikari.on(events.NewMessage(pattern="/start"))
async def strt(event):
    await event.reply(f"Hey❤️. I'm a 💐force subscribe bot💐 made specially for ❤️@{channel}!\n\nCheckout ❤️@The_SHIKARI_Network :)", buttons=[Button.url("Channel", url=f"https://t.me/{channel}"), Button.url("Repository", url="https://github.com/xditya/ForceSub")])

    
print("ForceSub Bot has started.\nDo visit @The_SHIKARI_Network!")
Shikari.run_until_disconnected()
